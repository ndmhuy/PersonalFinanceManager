...
\subsection{Custom Data Structures (No STL)}
We engineered two core generic container classes. These structures manage their own memory manually, replacing `std::vector` and `std::unordered\_map`.

\subsubsection{Dynamic Array (\texttt{ArrayList<T>})}
We implemented a template-based dynamic array to serve as the primary linear container for the application.
\begin{itemize}
    \item \textbf{Memory Strategy:} The class maintains a raw pointer \texttt{T* data} and a \texttt{capacity} counter.
    \item \textbf{Geometric Expansion:} When the array becomes full, the system allocates a new block of memory sized $2 \times \text{current\_capacity}$. It then copies existing elements to the new block and frees the old memory. This ensures the nature of an array.
    \item \textbf{Safety:} The class implements a RAII-compliant destructor that automatically invokes \texttt{delete[]} on the internal array, ensuring no memory leaks occur when the controller goes out of scope.
\end{itemize}

\subsubsection{Hash Map (\texttt{HashMap<K, V>})}
...
\begin{itemize}
    \item \textbf{Hashing Strategy:} We implemented the \textbf{djb2 algorithm} \cite{djb2} (created by Dan Bernstein) to map string keys to integer indices. This function initializes a hash value to 5381 and iterates through the string, updating the value using bitwise operations (`hash * 33 + char`). This approach is computationally efficient and generates a high-entropy distribution for ASCII strings (like our UUIDs).
    \item \textbf{Collision Resolution:} We utilized the \textbf{Separate Chaining} technique. Each "bucket" in the hash table points to a linked list of nodes. If two distinct keys hash to the same index, the new entry is safely appended to the chain.
    \item \textbf{Performance:} This reduces lookup complexity from $O(N)$ (linear scan) to $O(1)$ (average case), which is essential for keeping the application responsive as the dataset grows.
\end{itemize}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.7\linewidth]{img/UMLdiagram.png}
...
\subsection{Integration with the Technical Core}

While the \textbf{Technical Core} defines the fundamental data structures and algorithms,
the \texttt{AppController} acts as the orchestration layer that applies these mechanisms
to implement concrete application features.

Rather than re-implementing logic at the controller level, the system is designed so that
all critical operations in \texttt{AppController} directly leverage the guarantees provided
by the Technical Core.

\subsubsection{Sorted Transaction Management}

Whenever \texttt{AppController::AddTransaction} is invoked, the controller delegates the
ordering responsibility to the core transaction container.

\begin{itemize}
    \item The transaction list is maintained using the \textbf{sorted insertion by date}
    algorithm described in the Technical Core.
    \item This ensures that all downstream features (reports, date-range queries, dashboard summaries)
    operate on a globally ordered dataset without requiring additional sorting.
\end{itemize}

\textbf{Benefit:}  
The controller logic remains simple and readable, while chronological correctness is
guaranteed at the data-structure level.

\subsubsection{Fast Queries via Secondary Indices}

Filtering and lookup operations exposed by \texttt{AppController} (such as viewing transactions
by wallet, category, or income source) rely on the \textbf{secondary indexing mechanism}.

\begin{itemize}
    \item Upon insertion or deletion, \texttt{AppController} updates both the primary transaction list
    and the corresponding index maps.
    \item Query handlers then retrieve relevant subsets directly from these indices instead of
    performing full linear scans.
\end{itemize}

\textbf{Benefit:}  
This significantly improves responsiveness for user-driven queries while keeping controller
methods short and focused.

\subsubsection{Date Range Queries and Reporting}

Reporting features implemented in \texttt{NavReport} and aggregated by \texttt{AppController}
utilize the \textbf{range query on sorted data} algorithm.

\begin{itemize}
    \item The controller identifies the starting position using binary search.
    \item Only relevant transactions are scanned until the end date is exceeded.
\end{itemize}

\textbf{Benefit:}  
Reports scale with the number of matching records rather than the total dataset size,
making analytics predictable and efficient.

\subsubsection{Recurring Transaction Generation}

During application startup and scheduled checks, \texttt{AppController} processes
\texttt{recurring\_transactions} using the \textbf{incremental scheduling algorithm}
defined in the Technical Core.

\begin{itemize}
    \item Each blueprint is evaluated deterministically.
    \item Concrete transactions are generated exactly once per valid period.
\end{itemize}

\textbf{Benefit:}  
The controller avoids duplicate generation and maintains correctness without storing
redundant state.

\subsubsection{Search and Filtering Operations}

Search features exposed through \texttt{NavSearch} delegate evaluation logic to
predicate-based filters provided by the core layer.

\textbf{Benefit:}  
This separation allows search rules to evolve independently from navigation logic,
while remaining easy to test and reason about.

\subsubsection{Concurrency Guarantees}

All \texttt{AppController} methods that mutate shared state execute under the
\textbf{mutex protection} enforced by the Technical Core.

\textbf{Benefit:}  
Thread safety is achieved transparently, allowing background services (such as auto-save)
to coexist safely with user-driven actions.
